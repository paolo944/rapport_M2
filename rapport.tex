...
\begin{document}
	...
	\section{Security analysis}
		...
		\begin{itemize}
			\item \textit{Unbreakability}: the attacker recovers the secret key from the verification key(or public key). It is denoted \textbf{UB}.
			\item \textit{Universal Unforgeability}: the attacker can produce a valid signature of any message in the message space without necessarily recovering the verification key. It is denoted \textbf{UUF}.
			...
			It is denoted \textbf{EUF}.
		\end{itemize}
		Finally, there are 3 main adversarial models:
		\begin{itemize}
			\item \textit{Key-Only Attacks}: The adversary has only access to the verification keys, which is the simplest and unavoidable scenario. It is denoted \textbf{KOA}.
			\item \textit{Known-Message Attacks}: The adversary has access to a set of messages and their valid signatures. It is denoted \textbf{KMA}. 
			\item \textit{Chosen-Message Attacks}: The adversary can probe an oracle that can sign any message of his choice and he can request as much as he wants. It is denoted \textbf{CMA}.
		\end{itemize}
		...
		\subsection{Memory footprint of the attacks}
		
		One important thing to consider for practical implementations of attacks is the memory footprint. There exist a lot of algorithms that are either galactic, i.e. that have good asymptotic complexity but will never be used in practice because of big-Oh factors, famous examples of galactic algorithms are matrix multiplication in $\mathcal{O}(n^{2.371552})$ \cite{WXXZ23} or integer multiplication in $\mathcal{O}(nlog(n))$ \cite{HH21}. Other algorithms can be very practical but necessitate a huge amount of memory like birthday attacks or rainbow table attacks on hash functions thus making them impractical.
		
		In this section we discuss the memory needed by the Crossbred algorithm to attack the SBC signature scheme.
		
		First, we need to construct $Mac_{\leq D, \geq d, m}^k$ and $\mathcal{M}_{\leq D, \geq d, m}^k$, but the second is by definition a sub matrix of the first, so we would only need to construct the first. We can construct $\mathcal{M}_{\leq D, \geq d, m}^k$ by only manipulating pointers on $Mac_{\leq D, \geq d, m}^k$ for example. So, the important memory footprint that we need to consider in the first part is the one of $Mac_{\leq D, \geq d, m}^k$. Let us consider the following cases
		\begin{itemize}
			\item We use the sparse representation. Then we can use the CSR representation which needs to store an array of column index of the non-zero $col\_index$ elements and an array containing the number of non-zero elements per row $row\_index$. We do not need a third array of coefficients because all the non-zero coefficients are equal to 1.
			The size of $col\_index$ is $nnz$ and the size of $row\_index$ is the number of rows of the matrix.
			...
			$$
				memory\_footprint \leq \left(\left(\frac{n^2}{4} + \frac{n}{2} + 1\right) + 1\right) \times \left(\sum_{i=0}^{D}\binom{n}{i} - \sum_{i=0}^{d-2}\binom{k}{i}\right)m \times \lceil \lceil log\left(\sum_{i=0}^{D} \binom{n}{i}\right) \rceil \times \frac{1}{8} \rceil \text{ bytes}.
			$$
			\begin{example}
				If we have $D = 12, d = 10, n = 256, m = 257, k=176$, the memory\_footprint would be aboult
				$$
			...
			$$
			\begin{example}
				If we apply the same last example, this time the footprint would be
				$$
		...
		\subsubsection{Space complexity of other attacks}

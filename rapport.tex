...
\begin{document}
	...
	\section{MPCitH and SBC signature scheme}
		This section presents the MPCitH framework, the Subfield Bilinear Collision (SBC) problem, and the associated signature scheme.
		
		\subsection{Multiparty Computation}

		
		A standard example is to make construct a $(N-1)$-private arithmetic circuit among $N$ participants and they all broadcast their results. See figure \ref{fig:mpc}.
		...
		\begin{figure}[h]
			...
			\begin{tikzpicture}[
				...
				% Nœuds des participants avec images
				\node[participant] (A) at (0,0) {\includegraphics[width=\participantimgsize]{flash.png}};
				\node[participant] (B) at (2,-3) {\includegraphics[width=\participantimgsize]{hudson.png}};
				\node[participant] (C) at (5,-3) {\includegraphics[width=\participantimgsize]{martin.png}};
				\node[participant] (D) at (7,0) {\includegraphics[width=\participantimgsize]{king.png}};
				\node[participant] (E) at (2,3) {\includegraphics[width=\participantimgsize]{sally.png}};
				\node[participant] (F) at (5,3) {\includegraphics[width=\participantimgsize]{sheriff.png}};
				
				% Communications (flèches)
				\draw[<->] (A) -- (B);
				\draw[<->] (A) -- (C);
				\draw[<->] (A) -- (D);
				\draw[<->] (A) -- (E);
			...
			\end{tikzpicture}
			\caption{MPC protocol with broadcast communication}
			\label{fig:mpc}
		\end{figure}
		
		To evaluate the circuit that depends on the secret witness $w$, each party holds a secret share $[w]$. They perform local computations on their respective shares, then broadcast the results, which are subsequently combined to obtain the final output.
		...
		To represent the secret witness $w$, a linear secret sharing scheme is used among $N$ simulated parties. Each party $i$ receives a random share denoted by $[w]^{\llbracket i \rrbracket}$, and the prover computes an additional correction term defined as:
		$$
		\delta_w = w - \sum_{i=1}^{N} [w]^{\llbracket i \rrbracket}.
		$$
		...
		Since the introduction of the framework, many optimizations have been published to reduce the communication or the computation cost. We will discuss some of them.
		...
		In our case, we do not transmit the seed due to the use of linear secret sharing. Since we typically employ an $(N-1)$-private MPC protocol, we only share the co-path of the $i$-th leaf, which remains hidden from $\mathcal{V}$. As a result, instead of sharing $N$ values, we only need to share $log_2(N)$ values.

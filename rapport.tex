...
\begin{document}
	...
	\section{Security analysis}
		...
		\begin{itemize}
			...
			$$
				memory\_footprint \leq \left(\left(\frac{n^2}{4} + \frac{n}{2} + 1\right) + 1\right) \times \left(\sum_{i=0}^{D}\binom{n}{i} - \sum_{i=0}^{d-2}\binom{k}{i}\right)m \times log\left(\sum_{i=0}^{D} \binom{n}{i}\right) \text{ bytes}.
			$$
			\item If we use dense representation, then we could
		\end{itemize}

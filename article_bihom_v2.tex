...
\begin{document}
	...
	\section{Introduction}
		...
		\begin{proposition}[Proposition 3.2.5 in \cite{Bardet04}]
			...
			\begin{itemize}
				\item The sequence $f_1,\dots,f_m$ is semi-regular if and only if it's Hilbert's series is
				$$
					 \prod_{i=1}^{m}(1-z^d_i)/(1-z)^n,
				$$
				The series $S_{m, n}(z) = \sum_{d \geq 0}h_{d, m}(n)z^d = \prod_{i=1}^{m}(1-z^d_i)/(1-z)^n$ is called the generating series of $f_1,\dots,f_m$.
				\item The sequence $f_1,\dots,f_m$ is semi-regular on $\mathbb{F}_2$, if and only if it's Hilbert series is
				$$
				(1-z)^n/\prod_{i=1}^{m}(1+z^d_i),
				$$
				the series $S_{m, n}(z) = \sum_{d \geq 0}h_{d, m}(n)z^d = (1-z)^n/\prod_{i=1}^{m}(1+z^d_i)$ is called the generating series of $f_1,\dots,f_m$.
				\item Every permutation $f_{\sigma(1)},\dots,f_{\sigma(m)}$ est a semi-regular sequence.

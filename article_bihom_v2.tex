...
\begin{document}
	...
	\section{Preliminaries and notations}
		...
		\begin{itemize}
			...
			\item Let $R_{\leq s}$ be the polynomials of total degree $\leq s$. $I_{\leq s} = I \cap R_{\leq s}$.
		\end{itemize}
		...
		\begin{definition}[\cite{CLS}]
			Let $I$ be an ideal in $R$. The affine Hilbert function of $I$ is the function on the nonnegative integers s defined by
			$$
				{}^a HF_{R/I}(s) = dim R_{\leq s} / dim I_{\leq s} = dim R_{\leq s} - dim I_{\leq s}.
			$$
		\end{definition}
		...
		The smallest integer $s_0$ such that $HF_{R/I}(s_0) \leq 0$ is called the degree of regularity of the ideal $I$.

...
\begin{document}
	...
	\section{Preliminaries and notations}
		...
		Let $\mathbb{F}$ be a finite field and $\mathbb{K}$ a fiend not necessarily finite. 
		We denote the multivariate polynomial ring defined over a field in $n$ variables $\mathbb{K}[x_1,\dots,x_n] = R$ where $n$ is a positive integer.
		In the following we will use the standard notation for matrix multiplication complexity for a square matrix of size $n$, $\mathcal{O}(n^\omega)$,
	...
	\section{Signature Scheme}
		...
		\subsubsection{Faugère's F5}
		
		\subsubsection{Hilbert's series}
		...
		\begin{proposition}[Proposition 3.2.5 in \cite{Bardet04}]\label{PropSemi}
			...
			\begin{itemize}
				\item The sequence $f_1,\dots,f_m$ is semi-regular if and only if its Hilbert's series is the series $$S_{m, n}(z) = \sum_{d \geq 0}h_{d, m}(n)z^d = \prod_{i=1}^{m}(1-z^{d_i})/(1-z)^n$$ is called the generating series of $f_1,\dots,f_m$.
				\item The sequence $f_1,\dots,f_m$ is semi-regular on $\mathbb{F}_2$, if and only if its Hilbert series is the series $$S_{m, n}(z) = \sum_{d \geq 0}h_{d, m}(n)z^d = (1+z)^n/\prod_{i=1}^{m}(1+z^{d_i})$$ is called the generating series of $f_1,\dots,f_m$.
		...
		\begin{itemize}
			...
			\item[-] $I_{i-1} : f_i$ is the ideal spanned by $\{g \in R \mid gf_i \in I_{i-1}\}$. We denote the maximal minors of $M$ by $MaxMinors(M)$.
		\end{itemize}
